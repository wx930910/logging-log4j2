...
public class RedisManagerTest {
	...
	public void setUp() {
		initMocks();
		manager = new ManagerTestRedisAppenderBuilder().setHost("localhost").setPort(6379).setKeys(KEYS)
				.getRedisManager();
		manager.startup();
	}
	...
	private class ManagerTestRedisAppenderBuilder extends RedisAppender.Builder<ManagerTestRedisAppenderBuilder> {
		private RedisManager mockRedisManager1(LoggerContext loggerContext, String name, String keys, String host,
				int port) {
			RedisManager mockInstance = Mockito
					.spy(new RedisManager(loggerContext, name, keys.split(","), host, port, null, null));
			Mockito.doReturn(mockJedisPool).when(mockInstance).createPool(Mockito.any(), Mockito.anyInt(),
					Mockito.any());
			return mockInstance;
		}
		...
		RedisManager getRedisManager() {
			return mockRedisManager1(LoggerContext.getContext(), getName(), getKeys(), getHost(), getPort());
		}
	}
}
