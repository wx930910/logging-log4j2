...
public class PatternLayoutTest {
	public static ByteBufferDestination mockByteBufferDestination1() {
		ByteBuffer mockFieldVariableByteBuffer = ByteBuffer.wrap(new byte[2048]);
		ByteBufferDestination mockInstance = mock(ByteBufferDestination.class);
		when(mockInstance.drain(any()))
				.thenThrow(new IllegalStateException("Unexpected message larger than 2048 bytes"));
		doAnswer((stubInvo) -> {
	...
	private void assertEncode(final String expectedStr, final PatternLayout layout, final LogEvent event) {
		final ByteBufferDestination destination = PatternLayoutTest.mockByteBufferDestination1();
	...
	public void testHeaderFooterThreadContext() throws Exception {
		final PatternLayout layout = PatternLayout.newBuilder().setPattern("%d{UNIX} %m")
				.setConfiguration(ctx.getConfiguration()).setHeader("${ctx:header}").setFooter("${ctx:footer}").build();
	...
	public void testPatternSelector() throws Exception {
		...
		// @formatter:off
		final PatternSelector selector = MarkerPatternSelector.newBuilder().setProperties(patterns)
				.setDefaultPattern("%d %-5p [%t]: %m%n").setAlwaysWriteExceptions(true).setNoConsoleNoAnsi(true)
				.setConfiguration(ctx.getConfiguration()).build();
		// @formatter:on
		...
		final LogEvent event1 = Log4jLogEvent.newBuilder() //
				...
				.setMarker(MarkerManager.getMarker("FLOW")).setLevel(Level.TRACE) //
				.setIncludeLocation(true).setMessage(new SimpleMessage("entry")).build();
		final String result1 = new FauxLogger().formatEvent(event1, layout);
		final String expectPattern1 = String
				.format(".*====== PatternLayoutTest.testPatternSelector:\\d+ entry ======%n");
	...
	public void testRegexEmptyMarker() throws Exception {
		...
		final PatternLayout layout = PatternLayout.newBuilder()
				.setPattern("[%logger]%replace{[%marker]}{\\[\\]}{} %msg").setConfiguration(ctx.getConfiguration())
				.build();
		// Not empty marker
		final LogEvent event1 = Log4jLogEvent.newBuilder() //
				.setLoggerName(this.getClass().getName()).setLoggerFqcn("org.apache.logging.log4j.core.Logger") //
	...
	public void testEqualsMarkerWithMessageSubstitution() throws Exception {
		...
		final LogEvent event1 = Log4jLogEvent.newBuilder() //
				.setLoggerName(this.getClass().getName()).setLoggerFqcn("org.apache.logging.log4j.core.Logger") //
				.setLevel(Level.INFO) //
				.setMarker(MarkerManager.getMarker("TestMarker")).setMessage(new SimpleMessage("Hello, world!"))
				.build();
		...
		final LogEvent event2 = Log4jLogEvent.newBuilder() //
				.setLoggerName(this.getClass().getName()).setLoggerFqcn("org.apache.logging.log4j.core.Logger") //
				.setLevel(Level.INFO).setMessage(new SimpleMessage("Hello, world!")).build();
	...
	public void testLoggerNameTruncationByRetainingPartsFromEnd() throws Exception {
		{
			final PatternLayout layout = PatternLayout.newBuilder().setPattern("%c{1} %m")
					.setConfiguration(ctx.getConfiguration()).build();
			final LogEvent event1 = Log4jLogEvent.newBuilder().setLoggerName(this.getClass().getName())
					.setLoggerFqcn("org.apache.logging.log4j.core.Logger").setLevel(Level.INFO)
					.setMessage(new SimpleMessage("Hello, world 1!")).build();
			final String result1 = layout.toSerializable(event1);
			assertEquals(this.getClass().getName().substring(this.getClass().getName().lastIndexOf(".") + 1)
					+ " Hello, world 1!", new String(result1));
		}
		{
			...
			assertEquals(this.getClass().getName().substring(name.length() + 1) + " Hello, world 1!",
					new String(result1));
		}
		{
	...
	public void testCallersFqcnTruncationByRetainingPartsFromEnd() throws Exception {
		{
			...
			final LogEvent event1 = Log4jLogEvent.newBuilder().setLoggerName(this.getClass().getName())
					...
					.setSource(new StackTraceElement(this.getClass().getName(),
							"testCallersFqcnTruncationByRetainingPartsFromEnd",
							this.getClass().getCanonicalName() + ".java", 440))
					.build();
			final String result1 = layout.toSerializable(event1);
			assertEquals(this.getClass().getName().substring(this.getClass().getName().lastIndexOf(".") + 1)
					+ " Hello, world 1!", new String(result1));
