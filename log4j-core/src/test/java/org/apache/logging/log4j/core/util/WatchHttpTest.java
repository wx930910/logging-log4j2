...
public class WatchHttpTest {
	...
	public static ConfigurationListener mockConfigurationListener1(final Queue<String> queue, String name) {
		String mockFieldVariableName;
		Queue<String> mockFieldVariableQueue;
		ConfigurationListener mockInstance = Mockito.mock(ConfigurationListener.class);
		mockFieldVariableQueue = queue;
		mockFieldVariableName = name;
		Mockito.doAnswer((stubInvo) -> {
			mockFieldVariableQueue.add(mockFieldVariableName);
			return null;
		}).when(mockInstance).onChange(Mockito.any());
		return mockInstance;
	}
	...
	public void testWatchManager() throws Exception {
		...
		listeners.add(WatchHttpTest.mockConfigurationListener1(queue, "log4j-test1.xml"));
	...
	public void testNotModified() throws Exception {
		BlockingQueue<String> queue = new LinkedBlockingQueue<>();
		List<ConfigurationListener> listeners = new ArrayList<>();
		listeners.add(WatchHttpTest.mockConfigurationListener1(queue, "log4j-test2.xml"));
		TimeZone timeZone = TimeZone.getTimeZone("UTC");
		Calendar now = Calendar.getInstance(timeZone);
		Calendar previous = now;
		previous.add(Calendar.MINUTE, -5);
